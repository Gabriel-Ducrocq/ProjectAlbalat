...
class TestEmbedChunks:
    def test_compute_start_end_indexes(self):
        ...
        assert (
            indexes_0 == (0, 13291)
            and indexes_1 == (13291, 26582)
            and indexes_2 == (26582, 39875)
        ), f"""
    ...
    def test_load_model_wrong_dtype(self):
        ...
        try:
            load_model(
                "IEITYuan/Yuan-embedding-2.0-en", "cpu", "torch.bfloat654", "sdpa"
            )
    ...
    def test_load_model_device_cpu(self):
        model = load_model(
            "IEITYuan/Yuan-embedding-2.0-en", "cpu", "torch.float32", "sdpa"
        )
        assert str(model.device) == "cpu", (
            f"""The model should be on cpu, current {model.device}"""
        )

    def test_load_model_device_gpu(self):
        ...
        model = load_model(
            "IEITYuan/Yuan-embedding-2.0-en", device, "torch.bfloat16", "sdpa"
        )
    ...
    def test_load_model_dtype(self):
        if torch.cuda.is_available():
            device = "cuda:0"
        elif torch.mps.is_available():
            device = "mps:0"
